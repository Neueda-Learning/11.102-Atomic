...
public class AlertProcessing {
    ...
    // fetch all alerts
    public List<Alert> getAllAlerts(long accountId) {
        return alert.findAllByAccountNumber(accountId);
    }

    // fetch all alerts by their status
    public List<Alert> getAllAlertsByStatus(long accountId, int status) {
        return alert.findAllByAccountNumberAndStatusEquals(accountId, status);
    }

    // fetch all alerts by their rule id
    public List<Alert> getAllAlertsByRuleId(long accountId, long ruleId) {
        return alert.findAllByAccountNumberAndAlertIDEquals(accountId, ruleId);
    }

    public String acknowledgeAlert(Alert alert) {
        alert.setStatus(2);
        return "Alert Acknowledged!";
    }

    public String closeAlert(Alert alert) {
        alert.setStatus(3);
        return "Alert Closed!";
    }
